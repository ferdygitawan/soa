...
class Sale1 extends MY_Controller {
	...
	public function index() {
		$data['title'] = 'Payment';
		$data['sub_title'] = 'List';
		$data['sales1'] = $this->m_sale->get_all();
		$this->render('admin/sales1/index', $data);
	}
	...
	public function detail($id) {
		...
		$data['sale1'] = $this->m_sale->get_where('sale.id = '.$id); // get sale by id
		$data['payment_confs'] = $this->m_sale_payment_conf->get_where('sale_id = '.$id);
		$this->render('admin/sales1/detail', $data);
	}
		/**
	...
	public function create() {
		...
		for ($i=0; $i < $counter; $i++) { // product rules
			if ($this->input->post('product_id'.$i)) {
				...
				$this->form_validation->set_rules('diskon_price'.$i, 'Diskon '.$i, 'required|numeric|greater_than[0]');
			}
		}
		...
		if ($this->form_validation->run()) { // if pass
			if ($counter == 1) { // product is empty
				...
				$this->render('admin/sales1/create', $data);
				...
				redirect('admin/sale1/detail/','refresh');
			}
		} else { // if not pass
			$this->render('admin/sales1/create', $data);
		}
	...
	public function update_status($id) {
		if ($this->input->post()) {
			...
			redirect('admin/sale1/detail/'.$id,'refresh');
		} else {
			redirect('admin/sale1/detail/','refresh');
		}
	}
...
/* End of file Sale1.php */
/* Location: ./application/controllers/admin/Sale1.php */