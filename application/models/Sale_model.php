...
class Sale_model extends MY_Model {
	...
	public function get_customer_sale_id($id) {
		$this->db->select('*, 
											customer.name AS customer_name,');
		$this->db->join('customer', 'customer.id = sale.customer_id', 'left');
	...
	public function get_bkm_sale_id($id) {
		$this->db->select('*, 
											sale_payment_conf.bkm_no AS payment_number,');
		$this->db->join('sale_payment_conf', 'sale_payment_conf.sale_id = sale.id', 'left');
		$this->db->where('sale.id='.$id);
		$query = $this->db->get('sale');
		return $query->row()->payment_number;
	}
	...
	public function get_total_amount_by_sale_id($id) {
		$query = $this->db->query("SELECT SUM((product_price * product_qty)-((product_price * product_qty)*((diskon_price)/100))) total_amount FROM sale_detail WHERE sale_id = ".$id);
		...
		for ($i=1; $i < $counter; $i++) {
			if (!is_null($this->input->post('product_id'.$i))) {
				$data = [
					...
					'diskon_price' => $this->input->post('diskon_price'.$i),
